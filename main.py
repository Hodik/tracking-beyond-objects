...
recognizer_checkpoint = (
    "mmaction2/checkpoints/tsn_r50_1x1x3_100e_kinetics400_rgb_20200614-e508be42.pth"
)
    ...
    for box, cls, i in zip(boxes, clss, track_ids):
        if cls != person_class:
            if i in objects:
                ...
                objects[i] = ObjectTracker(
                    i,
                    box,
                    cls,
                    people,
                    objects,
                    frames_max_len=recognizer["sample_length"],
                )
...
def main():
    ...
    frames_inds = [frame_start + frame_interval * i for i in range(clip_len)]
    display_size = (w, h)
    ...
    while cap.isOpened():
        ...
        processed_frames = []
        success = True
        keyframe = {"frame": None, "boxes": None}
        ...
        while success and len(frames) < window_size:
            ...
            if success:
                ...
                processed_frame = mmcv.imresize(frame, stdet_input_size).astype(
                    np.float32
                )
                ...
                if len(frames) == window_size // 2:
                    ...
                    keyframe["frame"], keyframe["boxes"] = (
                        frame,
                        results[0].boxes[clss == person_class],
                    )
        ...
        if task.action_preds and keyframe["boxes"].id is not None:
            ...
            actions_buffer = {
                i: action for i, action in zip(track_ids, task.action_preds)
            }
        read_id += 1
